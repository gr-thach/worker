...
module.exports = (engineReport) => {
  if (
    ...
    engineReport.process.name === 'npm-audit'
  ) {
    let modOutput = [];
        ...
        for (var i = 0; i < modOutput.length; i++) {
          if (
            ...
            if (
              ...
              modOutput[i].metadata.patchedVersions != '<0.0.0'
            ) {
              ...
              const defaultVer = semver.coerce('0')
              const currentVer = semver.coerce(cleanCurrent) || defaultVer
              const candidateVer = semver.coerce(cleanCandidate) || defaultVer
              if (semver.lt(currentVer, candidateVer) || item.metadata.patchedVersions === '<0.0.0') {
